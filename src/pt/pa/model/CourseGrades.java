...
import pt.pa.Statistic;
import pt.pa.StatisticAverage;
...
public class CourseGrades {
    ...
    public CourseGrades(String course) {
        this.course = course;
        results = new HashMap<>();
        statistic = new StatisticAverage();
    }
    ...
    /**
     * @return value of current statistic
     */
    public double computeStatistic() {
        return statistic.compute((List<StudentGrade>) list());
    }
    ...
    public void add(StudentGrade g) throws CourseGradesException, NullPointerException {
        if (g == null) throw new NullPointerException("StudentGrade cannot be null.");
        if (results.containsKey(g.getId())) throw new CourseGradesException("Student already exists.");
    ...
    public int update(String studentId, int newGrade) throws CourseGradesException {
        ...
        if (!exists(studentId)) throw new CourseGradesException("Student does not exist: " + studentId);
